...
首次使用前，请检查并安装运行环境。计算引擎为纯 Python，无第三方依赖。

```bash
# 检查 Python 版本（需要 >= 3.9）
python3 --version

# 如果使用 conda，可创建独立环境（可选）
conda create -n captable-skills python=3.11 -y

# 如果需要运行测试，安装 pytest
pip install pytest
# 或 conda 环境下
conda run -n captable-skills pip install pytest
```

**注意**：计算引擎本身只依赖 Python 标准库，不需要安装任何第三方包。如果用户机器上已有 Python >= 3.9，可以直接运行：
```bash
python3 /path/to/captable-skills/main.py --list
```
...
```bash
python3 /path/to/captable-skills/main.py --scenario scenario1 --params '{
  "currentShareholders": [
...
```bash
python3 /path/to/captable-skills/main.py --scenario scenario1 --params-file /tmp/params.json
```
